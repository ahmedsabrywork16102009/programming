...
import { syncSkipperSettings } from './ad-skipper.js';
...
function handleVideoCommand(data) {
    ...
    if (mediaList.length === 0) mediaList = [...document.querySelectorAll('video, audio')];
    for (const m of mediaList) {
        ...
        } catch (e) { console.debug('WatchSmart: media command failed', e); }
    }
}
...
export function initBridge() {
    window.addEventListener('message', (event) => {
        ...
        if (type === 'WATCHSMART_SYNC') handleSpeedSync(event.data);
        else if (type === 'WATCHSMART_VIDEO_CMD') handleVideoCommand(event.data);
        else if (type === 'WATCHSMART_TOGGLE') state.enabled = !!event.data.enabled;
